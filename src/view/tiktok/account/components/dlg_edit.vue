<template>
  <el-dialog :visible.sync="$attrs.visible" title="更新" @opened="onOpened" width="60%"
...
import {accountUpdate} from "@/api/tiktok/account";
  ...
  methods: {
    ...
    async update() {
      this.$refs['elForm'].validate(async valid => {
        ...
        let resp = await accountUpdate({id: this.id, cookie: this.toUpdate.cookie})
        if (resp.code === 0) {
          this.$message("更新成功")
          this.$emit('close', false)
        }
