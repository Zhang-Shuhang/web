<template>
  <el-drawer :visible.sync="$attrs.visible" title="上架列表" @opened="onOpened" size="85%" :modal-append-to-body="true"
    ...
    <div class="search-term">
      <el-form class="table-form" ref="elForm" :model="searchInfo" size="mini" label-width="100px"
        ...
        <el-row :gutter="5">
          ...
          <el-col :xs="6" :sm="6" :lg="2">
            <el-form-item label-width="0">
              <el-input placeholder="产品id" v-model="searchInfo.simple_product_id" clearable filterable
          ...
          <el-col :xs="6" :sm="6" :lg="2">
            <el-form-item label-width="0">
              <el-input placeholder="产品名" v-model="searchInfo.simple_product_name" clearable filterable
                        @keyup.enter.native="onSubmit">
              </el-input>
            </el-form-item>
          </el-col>
          <el-col :xs="6" :sm="6" :lg="2">
            <el-form-item label-width="0">
              <el-select v-model="searchInfo.template_ids" placeholder="模板列表" multiple collapse-tags filterable
                         clearable :style="{width: '100%'}">
          ...
          <el-col :xs="6" :sm="6" :lg="2">
            <el-form-item label-width="0">
              <el-date-picker type="date" v-model="searchInfo.start_time" format="yyyy/MM/dd" value-format="yyyy/MM/dd"
                              :style="{width: '100%'}" placeholder="完成开始日期" unlink-panels
                              clearable></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :xs="6" :sm="6" :lg="2">
            <el-form-item label-width="0">
              <el-date-picker type="date" v-model="searchInfo.end_time" format="yyyy/MM/dd" value-format="yyyy/MM/dd"
                              :style="{width: '100%'}" placeholder="完成结束日期" unlink-panels
                              clearable></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :xs="6" :sm="6" :lg="2">
          ...
          <el-col :xs="6" :sm="6" :lg="2" v-if="selectRows.length > 0">
            <el-form-item label-width="0">
              <el-button type="warning" size="mini" :style="{width: '100%'}" @click="onRetry(selectRows)">重试
              </el-button>
            </el-form-item>
          </el-col>
          <el-col :xs="6" :sm="6" :lg="2" v-if="selectRows.length > 0">
            <el-form-item label-width="0">
              <el-button type="danger" size="mini" :style="{width: '100%'}" @click="selectCategory(selectRows)">
                变更账号类目
              </el-button>
            </el-form-item>
          </el-col>
      ...
      <el-table class="table-body" ref="multipleTable" :data="tableData" border @sort-change="sortChange"
        ...
        <el-table-column label="产品名" prop="product_name" align="center">
        </el-table-column>
        ...
        <el-table-column label="图片列表" prop="images" align="center" min-width="300px">
          <template slot-scope="{row}">
            <KdxImage style="display: inline-block" :src="img" v-for="(img, index) in row.images"
                      :key="index"></KdxImage>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="120px">
          <template slot-scope="{row}">
            <el-button style="width: 100%" size="mini" type="danger" @click="onRetry([row])"
                       v-if="!row.is_zf && row.executed && !row.success">重试
            </el-button>
            <el-button style="width: 100%" size="mini" type="danger" @click="onZf(row)"
                       v-if="!row.is_zf && !row.success">作废
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <TemuDlgVideoSelector :visible="dlgSelectVideo.visible" @close="dlgSelectVideo.visible=false"
                          @select="selectVideo"></TemuDlgVideoSelector>
    <TemuDlgProductCategoryProperties :default_account_id="dlgCategoryProperties.account_id" :can_change_account="false"
                                      :confirm="dlgCategoryProperties.confirm" :visible="dlgCategoryProperties.visible"
                                      @close="dlgCategoryProperties.visible=false"></TemuDlgProductCategoryProperties>
  </el-drawer>
</template>
...
export default {
  name: 'TiktokDlgSimpleProductUploads',
    ...
    onRetry(rows) {
      this.$confirm('请确认是否重试?', '提示', {
        ...
        type: 'warning'
          ...
          .then(async () => {
            let resp = await simpleProductUploadRetry({
              ids: rows.filter((item) => {
                return item.executed && !item.success
              }).map((item) => {
                return item.id
              })
            })
