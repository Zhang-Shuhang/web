...
import {accessoryCreate, accessoryLoad, accessoryUpdate} from "@/api/product/accessory";
import Upload from "@/view/components/upload.vue";

export default {
  name: 'TemuDlgAccessoryEdit',
  ...
  methods: {
    ...
    async onOpened() {
      ...
      let resp = await accessoryLoad({id: this.id})
      if (resp.code) {
    ...
    async onCreate() {
      let resp = await accessoryCreate({...this.form})
      if (resp.code) {
        return
      }
    ...
    async onUpdate() {
      let resp = await accessoryUpdate({...this.form})
      if (resp.code) {
        return
      }
