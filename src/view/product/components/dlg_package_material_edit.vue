...
import {packageMaterialCreate, packageMaterialLoad, packageMaterialUpdate} from "@/api/product/package_material";
import Upload from "@/view/components/upload.vue";

export default {
  name: 'TemuDlgPackageMaterialEdit',
  ...
  methods: {
    ...
    async onOpened() {
      ...
      let resp = await packageMaterialLoad({id: this.id})
      if (resp.code) {
    ...
    async onCreate() {
      let resp = await packageMaterialCreate({...this.form})
      if (resp.code) {
        return
      }
    ...
    async onUpdate() {
      let resp = await packageMaterialUpdate({...this.form})
      if (resp.code) {
        return
      }
