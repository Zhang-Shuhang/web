...
import {techCreate, techLoad, techUpdate} from "@/api/product/tech";
import Upload from "@/view/components/upload.vue";

export default {
  name: 'TemuDlgTechEdit',
  ...
  methods: {
    ...
    async onOpened() {
      ...
      let resp = await techLoad({id: this.id})
      if (resp.code) {
    ...
    async onCreate() {
      let resp = await techCreate({...this.form})
      if (resp.code) {
        return
      }
    ...
    async onUpdate() {
      let resp = await techUpdate({...this.form})
      if (resp.code) {
        return
      }
