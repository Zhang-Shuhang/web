<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-style="{'background-color': 'black', 'color': '#fff'}">
      ...
      <el-table-column label="能查看账号" min-width="150" align="center">
        <template slot-scope="scope">
          <el-select v-model="scope.row.account_ids" placeholder="能查看账号" @change="changeAccountIds(scope.row)"
                     allow-create multiple filterable clearable :style="{width: '100%'}">
            <el-option v-for="(item, index) in temuAccountOptions" :key="index" :label="`TEMU-${item.name}`"
                       :value="item.id"></el-option>
            <el-option v-for="(item, index) in sheinAccountOptions" :key="index" :label="`SHEIN-${item.name}`"
                       :value="item.id"></el-option>
            <el-option v-for="(item, index) in tiktokAccountOptions" :key="index" :label="`TIKTOK-${item.name}`"
                       :value="item.id"></el-option>
          </el-select>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="addUserDialog" custom-class="user-dialog" title="新增用户">
      <el-form :rules="rules" ref="userForm" :model="userInfo" label-width="120px">
        ...
        <el-form-item label="能查看账号" prop="account_ids">
          <el-select v-model="userInfo.account_ids" placeholder="能查看账号" allow-create multiple collapse-tags
...
export default {
  ...
  methods: {
    ...
    async changeAccountIds(row) {
      const res = await setUserAccountIds({
        uuid: row.uuid,
  ...
  async created() {
    ...
    if (accountListResp.code === 0) {
      this.temuAccountOptions = accountListResp.data
    }

    accountListResp = await sheinAccountList()
    if (accountListResp.code === 0) {
      this.sheinAccountOptions.push(...accountListResp.data)
    }

    accountListResp = await tiktokAccountList()
    if (accountListResp.code === 0) {
      this.tiktokAccountOptions.push(...accountListResp.data)
    }
  }
};
</script>
