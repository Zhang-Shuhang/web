  ...
  <div>
    <div class="search-term">
      <el-form class="table-form" ref="elForm" :model="searchInfo" size="mini" label-width="100px"
        ...
        <el-row :gutter="5">
          ...
          <el-col :xs="8" :sm="6" :lg="1">
            <el-button type="primary" size="mini" :style="{width: '100%'}" @click="openScannerReturnIdDlg">扫描
            </el-button>
          </el-col>
        </el-row>
      </el-form>
      ...
      <el-table class="table-body" ref="multipleTable" :data="tableData" border height="100%" @sort-change="sortChange"
        ...
        <el-table-column label="商品信息" width="200" align="center" fixed>
          <div slot-scope="{row}">
            <el-tag size="mini" type="primary" @click="copy(row.id)">{{row.id.length > 20 ? row.id.substring(0, 20) + ".." : row.id}}</el-tag>
...
<!--            <div>-->
<!--              <el-button type="primary" size="mini" style="width: 100%" @click="openAddToBuyPlan(row)" plain v-if="row.buy_record">生成采购(今日已采)</el-button>-->
<!--              <el-button type="warning" size="mini" style="width: 100%" @click="openAddToBuyPlan(row)" plain v-else>生成采购(今日未采)</el-button>-->
<!--            </div>-->
            <div v-if="row.histories.length">
              <el-button type="primary" size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" @click="reqStockBySku(row.id)" plain>申请备货(共享)</el-button>
            </div>
          </div>
        </el-table-column>
        <el-table-column label="店小秘" align="center">
          ...
          <el-table-column label="退回" align="center">
            <el-table-column label="已收" prop="total_return_count" sortable="custom" align="center">
              <div class="item-container" slot-scope="{row}">
                <div class="item" v-for="(history, index) in row.histories" :key="index">
                  {{history.total_return_count}}
                </div>
                <div class="footer">
                  {{row.total_return_count}}
                </div>
              </div>
            </el-table-column>
            <el-table-column label="未收" prop="total_unconfirmed_return_count" sortable="custom"
                             align="center">
              <div class="item-container" slot-scope="{row}">
                <div class="item" v-for="(history, index) in row.histories" :key="index">
                  {{history.total_unconfirmed_return_count}}
                </div>
                <div class="footer">
                  {{row.total_unconfirmed_return_count}}
                </div>
              </div>
            </el-table-column>
          </el-table-column>
          ...
          <el-table-column label="最多可加工" prop="jg_kcl_count" sortable="custom" align="center">
          </el-table-column>
          <el-table-column label="剩余+在途" prop="real_inventory" sortable="custom" align="center">
          </el-table-column>
        </el-table-column>
          ...
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              ...
              <div v-if="history.item && history.item.sale">
                <el-tag size="mini" type="primary" @click="copy(history.item.sale.product_skc_id.toString())">SKC: {{history.item.sale.product_skc_id}}</el-tag>
              </div>
              <div>
...
<!--              </div>-->
        ...
        <el-table-column label="操作" width="150" align="center" fixed="right">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              ...
              <div>
                <el-button type="primary" size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" @click="openPlans(history.id)">发货</el-button>
              </div>
              <div v-if="history.item && history.item.ban">
                <el-button size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" type="primary" @click="ban(history.id, false)">取消禁售</el-button>
              </div>
              <div v-else>
                <el-button size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" type="primary" @click="ban(history.id, true)">设置禁售</el-button>
              </div>
              <template v-if="history.item && history.item.sale && history.item.sale.picture_audit_status === 2">
                <el-row v-if="history.total_plan_count > 0">
                  <el-col :span="12">
                    <el-button type="primary" size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" @click="reqStock(history.item.sale.product_skc_id, history.id)" plain>备(Sku)</el-button>
                  </el-col>
                  <el-col :span="12">
                    <el-button type="primary" size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" @click="reqStock(history.item.sale.product_skc_id, 0)" plain>备(Skc)</el-button>
                  </el-col>
                </el-row>
                <div v-else>
                  <el-button type="primary" size="mini" style="width: 100%; height: 24px; line-height: 22px; padding: 0px" @click="reqStock(history.item.sale.product_skc_id, 0)" plain>申请备货(单Skc)</el-button>
                </div>
              </template>
            </div>
            <div class="footer"></div>
        ...
        <el-table-column prop="custom_available_sale_days" sortable="custom" align="center">
          ...
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.custom_available_sale_days.toFixed(2)}}
            </div>
            <div class="footer">
              {{row.custom_available_sale_days.toFixed(2)}}
            </div>
          </div>
        </el-table-column>
        <el-table-column label="平台可售天数" align="center">
          <el-table-column label="Temu" prop="available_sale_days_from_inventory" sortable="custom" align="center">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
                {{history.item.available_sale_days_from_inventory}}
              </div>
              <div class="footer">
              </div>
            </div>
          </el-table-column>
          <el-table-column label="我" prop="custom_available_sale_days_from_inventory" sortable="custom" align="center">
            <template slot="header">
              可售天数
        ...
        <el-table-column label="建议采购量" prop="custom_real_advice_quantity" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              <el-tag size="small" type="danger" effect="dark" v-if="history.custom_real_advice_quantity">
                {{history.custom_real_advice_quantity}}
              </el-tag>
              <el-tag size="small" type="success" v-else>
                {{history.custom_real_advice_quantity}}
              </el-tag>
            ...
            <div class="footer">
              {{row.custom_real_advice_quantity}}
            </div>
          </div>
        </el-table-column>
        <el-table-column label="Vmi" align="center">
          <el-table-column label="待发(量)" prop="wait_delivery_inventory_num" sortable="custom" align="center">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
        ...
        <el-table-column label="建议备货量" prop="advice_quantity" sortable="custom" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              <el-tag type="primary" >{{history.advice ? history.advice.max : 0}}</el-tag>
            </div>
            <div class="footer">
              {{row.advice_quantity}}
            </div>
        ...
        <el-table-column label="库存" align="center">
          ...
          <el-table-column label="不可用" align="center" prop="unavailable_warehouse_inventory_num" sortable="custom">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
          ...
          <el-table-column label="已发货" align="center" prop="wait_receive_num" sortable="custom">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
          ...
          <el-table-column label="待质检" align="center" prop="wait_qc_num" sortable="custom">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
          ...
          <el-table-column label="在途" align="center" prop="vmi_transportation_num" sortable="custom">
            <div class="item-container" slot-scope="{row}">
              <div class="item" v-for="(history, index) in row.histories" :key="index">
        ...
        <el-table-column label="今" prop="today" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.today}}
            </div>
        ...
        <el-table-column label="差" prop="two_day_diff" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.two_day_diff}}
            </div>
            <div class="footer">
              {{row.two_day_diff}}
            </div>
        ...
        <el-table-column label="昨" prop="two_day" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.two_day}}
            </div>
        ...
        <el-table-column label="差" prop="three_day_diff" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.three_day_diff}}
            </div>
            <div class="footer">
              {{row.three_day_diff}}
            </div>
        ...
        <el-table-column label="前" prop="three_day" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.three_day}}
            </div>
        ...
        <el-table-column label="7" prop="seven_day" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.seven_day}}
            </div>
        ...
        <el-table-column label="30" prop="thirty_day" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.thirty_day}}
            </div>
        ...
        <el-table-column label="全" prop="all" sortable="custom" sort-by="today" align="center">
          <div class="item-container" slot-scope="{row}">
            <div class="item" v-for="(history, index) in row.histories" :key="index">
              {{history.all}}
            </div>
...
<!--          <div class="item-container" slot-scope="{row}">-->
<!--            <div class="item" v-for="(history, index) in row.histories" :key="index">-->
<!--              <el-button type="primary" size="mini" style="width: 100%" @click="openPlans(history.id)" plain>发货</el-button>-->
<!--            </div>-->
<!--            <div class="footer">-->
<!--            </div>-->
<!--          </div>-->
...
export default {
  ...
  async created() {
    ...
    this.$set(this.searchInfo, "other_cond", [23, 33, 62])
