<template>
  <div>
    <div class="search-term">
      ...
      <el-table class="table-body" ref="multipleTable" :data="tableData" border height="100%" @sort-change="sortChange"
        ...
        <el-table-column align="center" label="匹配" prop="prefixes">
          <template slot-scope="{row}">
            <template v-if="row.prefixes">
              <div v-for="(p, index) in row.prefixes" :key="index">
                <el-tag style="margin-left: 5px" size="mini" type="primary"> 前缀: {{ p }} </el-tag>
              </div>
            </template>
            <template v-else>
              <div v-for="(p, index) in row.sku_ids" :key="index">
                <el-tag style="margin-left: 5px" size="mini" type="primary">
                  Sku Id: {{ p }}
                </el-tag>
              </div>
            </template>
        ...
        <el-table-column align="center" label="实拍图" prop="real_shout_image">
          <template slot-scope="{row}">
            <KdxImage :src="row.real_shout_image" width="100%" height="100%" v-if="row.real_shout_image"></KdxImage>
          </template>
        </el-table-column>
        <el-table-column align="center" label="实拍图2" prop="real_shout_image">
          <template slot-scope="{row}">
            <KdxImage :src="row.real_shout_image2" width="100%" height="100%" v-if="row.real_shout_image2"></KdxImage>
          </template>
        </el-table-column>
        <el-table-column align="center" label="实拍图3" prop="real_shout_image">
          <template slot-scope="{row}">
            <KdxImage :src="row.real_shout_image3" width="100%" height="100%" v-if="row.real_shout_image3"></KdxImage>
          </template>
        </el-table-column>
        <el-table-column align="center" label="说明书" prop="guide_file_images">
          <template slot-scope="{row}">
            <KdxImage :src="image" style="display: inline-block" v-for="(image, index) in row.guide_file_images" :key="index"></KdxImage>
          </template>
        </el-table-column>
      ...
      <TemuDlgLabelCodeRule :is_create="dlg_rule.is_create" :is_copy="dlg_rule.is_copy" :row="dlg_rule.row" :visible="dlg_rule.visible" @close="dlg_rule.visible = false" @update="getTableData"></TemuDlgLabelCodeRule>
    </div>
  </div>
...
import {labelCodeRuleList, labelCodeRuleRemove} from "@/api/temu/label_code_rule";
import TemuDlgLabelCodeRule from "@/view/temu/label_code_rule/dlg_rule.vue";

export default {
  components: {
    TemuDlgLabelCodeRule,
    KdxImage
  },
  mixins: [infoList],
  watch: {
