    ...
    <el-form class="table-form" ref="elForm" :model="info" size="mini" label-width="90px" label-position="left" v-if="info">
      ...
      <el-form-item label="验证码">
        <el-row :gutter="5">
          ...
          <el-col :span="12">
            <el-button type="primary" style="width: 100%;" @click="send">获得验证码({{info.bankPhoneNum}})</el-button>
          </el-col>
        </el-row>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async onOpened() {
      ...
      this.info = resp.data
      if (this.is_semi) {
        this.apply_amount = this.info.availableBalance - 5000
        this.apply_amount -= this.apply_amount % 10000
      } else {
