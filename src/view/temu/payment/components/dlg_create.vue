<template>
  <el-dialog :visible.sync="$attrs.visible" title="创建" @opened="onOpened" width="60%"
    ...
    <el-form class="table-form" :rules="toCreateRules" ref="elForm" :model="toCreate" size="mini" label-width="100px" label-position="right">
      ...
      <el-form-item label-width="0">
        <el-input type="textarea" :rows="5" v-model="toCreate.cookie" placeholder="cookie" clearable></el-input>
      </el-form-item>
      <el-form-item label-width="0">
...
import {accountCreate} from "@/api/temu/account";
...
export default {
  ...
  methods: {
    ...
    async onOpened() {
      ...
      this.toCreate.name = ""
      this.toCreate.cookie = ""
    },
    async create() {
      this.$refs['elForm'].validate(async valid => {
        if (!valid) return

        let resp = await accountCreate({id: Number(this.toCreate.id), name: this.toCreate.name, cookie: this.toCreate.cookie})
        if (resp.code === 0) {
          this.$message("创建成功")
          this.$emit('close', false)
        }
