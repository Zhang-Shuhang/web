<template>
  <el-dialog :visible.sync="$attrs.visible" :title="`申请备货(单${sku_id ? 'Sku' : 'Skc'})`" @opened="onOpened" width="80%"
...
<script>
import {query, create} from "@/api/temu/stock";
...
export default {
  ...
  methods: {
    ...
    async onOpened() {
      ...
      let resp = await query({id: this.id})
      if (resp.code) {
        return
      }

      for (const v of resp.data) {
        if (this.sku_id && v.productSkuId !== this.sku_id) {
          continue
        }
    ...
    async apply() {
      ...
      for (const v of this.toSet.list) {
        quantity_map.set(v.productSkuId, v.to_send)
      }
      let resp = await create({skc_id: this.id, quantity_map: Object.fromEntries(quantity_map), times: this.times, split: this.split})
      if (resp.code === 0) {
        this.$message("申请成功")
        this.$emit('close', false)
      }
