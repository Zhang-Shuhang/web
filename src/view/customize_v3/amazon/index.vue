      ...
      <el-form class="table-form" ref="elForm" :model="searchInfo" size="mini" label-width="100px"
        ...
        <el-row :gutter="5">
          ...
          <el-col :xs="12" :sm="6" :lg="3">
            <el-form-item label-width="0">
              <el-input placeholder="包裹号，店小秘 XM 单号" v-model="searchInfo.package_ids" clearable filterable
...
import {customizeAmazonOrderList, customizeAmazonOrderPb, customizeAmazonOrderSync} from "@/api/customize_v3/amazon";
...
export default {
  ...
  data() {
    return {
      listApi: customizeAmazonOrderList,
  ...
  methods: {
    onPb() {
      ...
      this.$confirm('是否确认排版?', '提示', {
        ...
        let resp = await customizeAmazonOrderPb({ids: ids})
        if (resp.code) {
          return
        }
    ...
    async syncCustomizeOrder() {
      let resp = await customizeAmazonOrderSync()
      if (resp.code) {
        return
      }
