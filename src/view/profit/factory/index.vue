<template>
    <div>
        ...
        <el-table :data="tableData" stripe style="width: 100%"
            ...
            <el-table-column align="center" prop="sku" label="货号" min-width="100px">
                <template slot-scope="{row}">
                    <el-tag size="small" type="primary">{{ row.sku }}</el-tag>
                    <i class="el-icon-data-line" style="color: red; margin-left: 10px"
                        @click="openDataLineDlg(row.sku)"></i>
                </template>
            </el-table-column>
        ...
        <KdxDlgProfitSkuDataLine :sku="dlgProfitDataLine.sku" :visible="dlgProfitDataLine.visible"
            @close="dlgProfitDataLine.visible = false"></KdxDlgProfitSkuDataLine>
    </div>
</template>
<script>
import { profitStatisticsListFactory } from "../../../api/profit/statistics";
import KdxDlgProfitSkuDataLine from "@/view/profit/components/dlg_sku_data_line.vue";
import Daily from "@/view/components/daily.vue";
import DateSelectTabs from "@/view/profit/components/date_select_tabs.vue";

export default {
    components: { Daily, KdxDlgProfitSkuDataLine, DateSelectTabs },
    data() {
        return {
            tableData: [],
            dlgProfitDataLine: {
    ...
    methods: {
        async getStatisticsList(start_time, end_time) {
            let resp = await profitStatisticsListFactory({ start_time, end_time })
            if (resp.code === 0) {
        ...
        openDataLineDlg(sku) {
            this.dlgProfitDataLine.sku = sku
