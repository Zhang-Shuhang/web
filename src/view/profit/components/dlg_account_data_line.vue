<template>
  <el-drawer :visible.sync="$attrs.visible" title="账号数据" @opened="onOpened" size="50%"
...
import {profitListAccount} from "@/api/profit/list";

export default {
  name: 'KdxDlgProfitAccountDataLine',
  ...
  methods: {
    ...
    async onOpened() {
      ...
      params.account_ids = [this.id]
      let resp = await profitListAccount(params)
      if (resp.code) {
        return
      }
